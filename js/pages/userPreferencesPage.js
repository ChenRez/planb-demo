...
function savePreferences() {
  ...
  if (fromSignup) {
    ...
    if (!pending.email) {
      showToast("פרטי ההרשמה לא נמצאו, אנא חזרי לדף ההרשמה");
      window.location.href = "signup.html";
      return;
    }
    ajaxCall("POST", "/api/auth/register", pending,
      ...
      function (err) {
        if (err.status === 409) {
          showToast("כתובת האימייל כבר קיימת במערכת, אנא חזרי לדף ההרשמה");
        } else {
    ...
    ajaxCall("PUT", "/api/preferences/" + userId, prefs,
      function () { window.location.href = "home.html"; },
      function () { showToast("שגיאה בשמירת ההעדפות, נסי שוב"); }
    );
  }
}
