...
function updateSalesStats(data) {
  var sold = 0, active = 0, earned = 0;
  for (var i = 0; i < data.length; i++) {
    var s = data[i].status;
    if (s === "Sold") {
      sold++;
      earned += Number(data[i].priceRequested) || 0;
    } else if (s === "Public" || s === "Highlighted" || s === "Price_Drop") {
      active++;
    }
  }
  var $nums = $(".stat-num");
  if ($nums.length >= 3) {
    $nums.eq(0).text("\u20AA" + earned.toLocaleString("he-IL"));
    $nums.eq(1).text(sold);
    $nums.eq(2).text(active);
  }
}
...
function deleteListing(event, listingId) {
  ...
  ajaxCall("DELETE", "/api/listings/" + listingId + "?userId=" + userId, null,
    ...
    function (xhr) { showToast((xhr.responseJSON && xhr.responseJSON.message) || "שגיאה במחיקת הפרסום"); }
  );
}
