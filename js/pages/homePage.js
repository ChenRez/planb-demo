...
/* שחזור מיקום הגלילה בחזרה מדף פרטי מוצר (הערה 13) */
function restoreHomeScroll() {
  try {
    var y = sessionStorage.getItem("homeScrollTop");
    if (y === null) return;
    sessionStorage.removeItem("homeScrollTop");
    var sc = document.querySelector(".scroll-content");
    if (!sc) return;
    requestAnimationFrame(function () {
      sc.scrollTop = parseInt(y, 10) || 0;
    });
  } catch (e) {}
}

/* גלילה חזרה לראש הדף (הערה 15) */
...
function renderForYouCards(forYou) {
  ...
  forYou.forEach(function(ev) {
    var defImg = escapeHtml(fallbackImg());
    container.innerHTML += `
      <div class="event-card" onclick="goToProduct(${ev.id})">
        <div class="card-image-wrap">
          <img src="${escapeHtml(ev.img)}" class="card-bg" onerror="this.onerror=null; this.src='${defImg}';" />
        ...
        <div class="card-content">
          ...
          <div class="card-info-row">
            <span class="material-symbols-rounded calendar-icon-styled" style="font-size:18px">confirmation_number</span>
            <span>${ticketCountLabel(ev.qty)}</span>
          </div>
          <div class="card-footer-row">
            <div class="price-group">
              <span class="price-label">לכרטיס</span>
...
function switchForYouTab(tab) {
  ...
  var tabAllEl = document.getElementById('tabAll');
  if (tabAllEl) tabAllEl.classList.toggle('active', tab === 'all');

  // "הכל" — הצגת כל המודעות בגלילה אנכית (הערה 14)
  if (tab === 'all') {
    ...
    container.innerHTML += `
      <div class="v-card" onclick="goToProduct(${ev.id})">
        <div class="v-card-img-wrap">
          <img src="${escapeHtml(ev.img)}" class="v-card-img" onerror="this.onerror=null; this.src='${defImg}';" />
        ...
        <div class="v-card-content">
          ...
          <div class="v-date">
            <span class="material-symbols-rounded calendar-icon-styled">confirmation_number</span>
            <span>${ticketCountLabel(ev.qty)}</span>
          </div>
          <div class="v-card-footer">
            <div class="v-price">${ev.price}₪ <span style="font-size:10px;font-weight:500;opacity:.7;">לכרטיס</span></div>
...
function goToProduct(id) {
  try {
    var sc = document.querySelector(".scroll-content");
    if (sc) sessionStorage.setItem("homeScrollTop", sc.scrollTop);
  } catch (e) {}
  window.location.href = `productDetails.html?id=${id}`;
}

// תווית מספר כרטיסים לכרטיס מודעה
function ticketCountLabel(q) {
  q = parseInt(q) || 1;
  return q === 1 ? "כרטיס בודד" : q + " כרטיסים";
}
...
function goBackToHome() {
  ...
  // איפוס מצב הטאבים של "בול בשבילך" (הערה 14)
  _currentForYouTab = 'personalized';
  var tp  = document.getElementById('tabPersonalized'); if (tp)  tp.classList.add('active');
  var pop = document.getElementById('tabPopular');      if (pop) pop.classList.remove('active');
  var ta  = document.getElementById('tabAll');          if (ta)  ta.classList.remove('active');
  selectedCategories.clear();
  selectedCategories.add("הכל");
