...
function showWelcomeToast(firstName) {
  var toast = $('<div>').css({
    ...
    '<div style="font-size:32px;margin-bottom:10px;color:#ed5f65"><i class="fas fa-circle-check"></i></div>' +
  ...
  ajaxCall("POST", "/api/auth/register", data,
    ...
    function (err) {
      ...
      if (err.status === 409) {
        ...
        showToast((err.responseJSON && err.responseJSON.message) || "נתונים חסרים או שגויים");
      } else {
