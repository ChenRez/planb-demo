...
document.addEventListener("DOMContentLoaded", function () {
  ...
  ajaxCall("GET", "/api/users/" + userId, null,
    function (user) {
      ...
      updateRatingDisplay(user.trustScore, user.totalSales, user.totalPurchases);
...
function updateRatingDisplay(score, totalSales, totalPurchases) {
  var ratingBox = document.getElementById("ratingBox");
  var sales     = totalSales     || 0;
  var purchases = totalPurchases || 0;
  ...
  var stars      = Math.round(score * 10) / 10;
  ...
  document.getElementById("ratingCount").textContent  = sales + " מכירות · " + purchases + " רכישות";
}
...
function savePayment() {
  ...
  ajaxCall("PUT", "/api/users/" + userId + "/payment",
    ...
    function () { showToast("שגיאה בשמירת פרטי תשלום"); }
  );
}
...
function saveBank() {
  ...
  ajaxCall("PUT", "/api/users/" + userId + "/bank",
    ...
    function () { showToast("שגיאה בשמירת פרטי בנק"); }
  );
}
...
function uploadAvatar(input) {
  ...
  ajaxCall("POST", "/api/users/" + userId + "/avatar?requesterId=" + userId, formData,
    function (res) { showAvatar(res.url); },
    function (xhr) {
      showToast((xhr && xhr.responseJSON && xhr.responseJSON.message) || "שגיאה בהעלאת התמונה");
    }
  );
  input.value = "";
}
...
function deleteAvatar() {
  ...
  ajaxCall("DELETE", "/api/users/" + userId + "/avatar?requesterId=" + userId, null,
    ...
    function (xhr) {
      showToast((xhr && xhr.responseJSON && xhr.responseJSON.message) || "שגיאה במחיקת התמונה");
    }
  );
}
