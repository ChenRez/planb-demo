...
function doSearch() {
  ...
  if (minP !== "" && maxP !== "" && Number(minP) > Number(maxP)) {
    showToast("טווח המחירים לא תקין: מחיר מינימלי גדול מהמקסימלי");
    return;
  }
...
function searchSuccess(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var qNum = parseInt(item.quantity) || 1;
    var qtyTxt = qNum === 1 ? "כרטיס בודד" : qNum + " כרטיסים";
    ...
    container.innerHTML +=
      '<div class="result-card" onclick="window.location.href=\'productDetails.html?id=' + item.listingId + '\'">' +
        ...
        '<div class="result-info">' +
          ...
          '<div class="result-loc">' + location + ' · ' + qtyTxt + '</div>' +
          '<div class="result-bottom">' +
            '<span class="result-cat">' + cat + '</span>' +
            '<span class="result-price">' + price + '₪ <span style="font-size:11px;font-weight:500;opacity:.7;">לכרטיס</span></span>' +
          '</div>' +
        '</div>' +
      '</div>';
  }
