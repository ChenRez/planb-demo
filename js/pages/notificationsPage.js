...
function buildNotifItem(notif) {
    ...
    $item.on("click", function () {
        ...
        const goToListing = function () {
            if (notif.listingId) {
                window.location.href = "productDetails.html?id=" + notif.listingId;
            }
        };
    ...
    $deleteBtn.on("click", function (e) {
        ...
        ajaxCall("DELETE", "/api/notifications/" + id, null,
            ...
            function () { showToast("שגיאה במחיקת ההתראה"); }
        );
    });
...
$(document).ready(function () {
    ...
    // כניסה למסך ההתראות = "נצפו" → מנקה את העיגול האדום על הפעמון
    if (typeof markNotificationsSeen === "function") markNotificationsSeen();
    if (typeof setNotificationBadge === "function") setNotificationBadge(0);
